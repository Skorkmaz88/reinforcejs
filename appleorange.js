...
World.prototype = {
  ...
  tick: function() {
    ...
    for(var i=0,n=this.items.length;i<n;i++) {
        ...
        if(d < it.rad + a.rad) {
          ...
          if(!rescheck) {
            ...
            if(it.type === 1) {
              if(a.type == 2)
              {
                a.digestion_signal += 0.2; // nah apple
            ...
            if(it.type === 3) {
              ...
              if(a.type == 2)
              {
                a.digestion_signal += 1.0; // wow orange

                console.log("orange eater got orange");
              }

              else
              {
                a.digestion_signal += 0.0; // mmm not so good
              }
                a.oranges++;
      ...
      */
      if(it.age > 200 ) {
        ...
        update_items = true; // Changed it skorkmaz
      }
    ...
    // Item adder
    if(this.items.length < 100 && this.clock % 5 === 0 ) {

      var newitx = randf(20, this.W-20);
      var newity = randf(20, this.H-20);

      //var newitx = this.agents[1].p.x;
      //var newity = this.agents[1].p.y;
      var newitt = 1 // Always food randi(1, 3); // food or poison (1 and 2)
      var newit = new Item(newitx, newity, newitt);
      this.items.push(newit);
      var x = randi(1, 5);
      if (x < 3)
      {
        var newitx2 = randf(20, this.W-20);
        var newity2 = randf(20, this.H-20);
      //var newitx2 = this.agents[1].p.x - 50;
      //var newity2 = this.agents[1].p.y - 50;
      var newitt2 = 2 // Always poison randi(1, 3); // food or poison (1 and 2)
      var newit2 = new Item(newitx2, newity2, newitt2);


      var newitx3 = randf(20, this.W-20);
      var newity3 = randf(20, this.H-20);

      //var newitx3 = this.agents[1].p.x + 50;
      //var newity3 = this.agents[1].p.y + 50;
      var newitt3 = 3 // Always orange randi(1, 3); // food or poison (1 and 2)
      var newit3 = new Item(newitx3, newity3, newitt3);
      this.items.push(newit2);
      this.items.push(newit3);
      }
...
var Eye = function(angle) {
  ...
  this.max_range = 240;
  this.sensed_proximity = 240; // what the eye is seeing. will be set in world.tick()
